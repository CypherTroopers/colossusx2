...
func allocCUDAManaged(size uint64) (managedAllocation, error) {
	...
	_ = C.colossusx_cuda_mem_advise(ptr, C.size_t(size), C.int(C.cudaMemAdviseSetPreferredLocation), C.int(0))
	_ = C.colossusx_cuda_prefetch(ptr, C.size_t(size), C.int(0))
